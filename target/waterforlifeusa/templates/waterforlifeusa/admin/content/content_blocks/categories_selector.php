...
function process_selected_categoeries(){
   ...
   url: "<? print site_url('ajax_helpers/taxonomy_taxonomyGetParentIdsForId') ?>",
   ...
   success: function(data){
	  ...
	  $brokenstring=data.split(',');
		...
		var find_c = tax_parent.find(":checked");
			...
			$.post("<? print site_url('ajax_helpers/taxonomy_categories_print_breadcrumbs_path_for_categories_ids') ?>", {
			category_ids: temp,
	   ...
	   $("#validation_valid_categories").val('yes');
			...
			$.post("<? print site_url('ajax_helpers/taxonomy_categories_get_main_site_section_id_for_caegory_ids') ?>", {  
			category_ids: temp
			 },
			  function(data){
			   $("#content_parent").val(data);
					...
					if (document.getElementById("content_url_demo_holder") != null){
			   		//lets get the url
							$.post("<? print site_url('ajax_helpers/content_get_url_by_id') ?>", {  
							id: data
							 },
							  function(data1){
...
<div id="add_content_to_more_categories_dialog_container" style="display:none;">
       <?
	$link = 'javascript:add_selected_categories({id})';
	 ...
	 ?>
      <div class="ooyes_ul_tree_container" treeparams='<? print $tree_params_string;  ?>'></div>
...
<input name="content_parent"  id="content_parent" type="hidden" value="<? print $form_values['content_parent'] ?>"   />
