...
$.post("<? print site_url('admin/media/contentMediaPicturesList') ?>/to_table:table_content/queue_id:"+media_upload_queue_pic+"/to_table_id:"+media_pictures_to_table_id+"/random_stuff:"+Math.random(), function(data){
...
/*	var media_upload_queue_pic = $('#media_queue_pictures').val();
	...
	var settings_object_pics = {
	  upload_url : '<? print site_url('admin/media/mediaUploadPictures') ?>/to_table:table_content/queue_id:'+media_upload_queue_pic+'/to_table_id:'+media_pictures_to_table_id,
	  flash_url : "<?php print_the_static_files_url() ; ?>js/swfupload/swfupload.swf",
	 // post_params: {"PHPSESSID" : "<?php echo session_id(); ?>"},
	 //post_params: {"PHPSESSID" : "<? print $this->session->userdata('session_id'); ?>"},
	 post_params: {"<?php echo $this->config->item('sess_cookie_name'); ?>" :"<?php echo $this->session->get_cookie_data(); ?>", "PHPSESSID" : "<? print $this->session->userdata('session_id'); ?>"},
...
*/
      ...
      <div>
        ...
        <div style="padding-left: 5px;"> <span id="spanButtonPlaceholder_pic"></span>
         <iframe frameborder="0" src="<? print site_url('admin/media/mediaUploadPicturesIframe') ?>/to_table:table_content/queue_id:<?php print intval( $form_values['id']) ; ?>_<? print $this->session->userdata('session_id'); ?>_media_queue_videos/to_table_id:<?php print $form_values['id']; ?>" height="150" width="450" ></iframe>
         <br />
        </div>
      </div>
      <br />
      <input name="media_pictures_to_table_id" type="hidden" id="media_pictures_to_table_id" value="<?php print $form_values['id']; ?>">
      <input name="media_queue_pictures" type="hidden" id="media_queue_pictures" value="<?php print $form_values['id']; ?>_<? print $this->session->userdata('session_id'); ?>_media_queue_pictures"></td>
  </tr>
  <tr>
