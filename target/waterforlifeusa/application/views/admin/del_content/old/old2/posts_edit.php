...
<form action="<? print $this->uri->uri_string(); ?>" class="pull" method="post" enctype="multipart/form-data">
  ...
  <h1 class="block" style="padding:7px 0"><?php if( $className == 'content' and $the_action == 'posts_add')  : ?>Add new post<? endif; ?><?php if( $className == 'content' and $the_action == 'posts_edit')  : ?>Edit: <?php print character_limiter($form_values['content_title'], 300, ' ') ; ?><? endif; ?></h1>
  <div id="tabs">
    <ul class="tabs">
	...
	?>
        ...
        <? // var_dump($section); ?>
        <label>
          <input name="content_parent" type="radio" value="<? print $section['id'] ?>"  <? if($form_values['content_parent'] == $section['id']) : ?>   checked="checked"  <? endif; ?>  />
...
$.post("<? print site_url('admin/content/contentGenerateTagsForPost') ?>", {  data: some_data },
 ...
 $("#taxonomy_tags_csv").val($the_val+ ", "+ $tag);
        ...
        <textarea name="taxonomy_tags_csv" id="taxonomy_tags_csv" wrap="virtual" cols="20" rows="10"><? print $thetags; ?></textarea>
	...
	?>
        <table border="0" cellspacing="5" cellpadding="5"> 
          ...
          <tr>
            <td><? /// foreach($avalable_tags as $tag): ?>
              <!--<a href="javascript:tags_append_csv('<? print $tag; ?>')"><? print $tag; ?></a>,-->
              <? // endforeach; ?>
...
<!-- <tr>
  ...
  <tr>
    ...
    <? if($form_values['is_from_rss'] == 'y' ) : ?><img src="<?php print_the_static_files_url() ; ?>/icons/feed__arrow.png" alt=" " border="0"><? endif; ?>
