<script type="text/javascript">
	...
	if (answer){
		$.post("<? print site_url('admin/menus/menus_delete_menu_item') ?>", { delete_menu_item: $delete_menu_item },
  function(data){
 //alert(data); 
 $("#menu_item_id_" +$delete_menu_item ).addClass("light_red_background");
  $("#menu_item_id_" +$delete_menu_item  ).fadeOut();
	...
	var answer = confirm("Sure?")
	if (answer){
	//window.location= '<? print site_url($this->uri->uri_string()); ?>/delete:'+ $id ; 
	$.post("<? print site_url('admin/menus/menus_delete') ?>/delete:"+$id, { name: "John", time: "2pm" },
  function(data){
	...
	$.get("<? print site_url('admin/menus/menus_show_menu_ajax') ?>/id:"+$id, { name: "John", time: "2pm" },
  function(data){
...
<div class="boo tablesClass">
  <table border="0" cellspacing="0" cellpadding="0" width="100%" style="border: none">
    ...
    <tr id="main_menus_container_id_<?php print $item['id']; ?>">
      <td style="border: none"><form action="<? print site_url($this->uri->uri_string()); ?>" method="post" enctype="multipart/form-data">
        ...
        <? $menu_items = $this->content_model->getMenuItems($item['id']); ?>
        <? if(!empty($menu_items)): ?>
        <? foreach($menu_items as $menu_item): ?>
        <form  action="<? print site_url('admin/menus/menus_save_menu_item') ?>" method="post" id="menu_item_form_<?php print $menu_item['id'] ;  ?>" enctype="multipart/form-data">
            ...
            <tr>
              ...
              <td><? if($menu_item['content_id'] != 0):  ?>
                ...
                <div id="menu_item_form_<?php print $menu_item['id'] ;  ?>_menu_url_placeholder">
                  <input name="dummy_1" type="text" value="<? print $url; ?>" />
                </div>
                <a href="<? print site_url('admin/menus/menus_edit_small_menu_item') ?>/id:<?php print $menu_item['id']; ?>/form:menu_item_form_<?php print $menu_item['id'] ;  ?>/?height=500&width=400&modal=false" class="thickbox" title="Change item">change</a>
              ...
              <td><a href="<? print site_url('admin/menus/index'); ?>/move_up:<?php print $menu_item['id'] ;  ?>">up</a></td>
              <td><a href="<? print site_url('admin/menus/index'); ?>/move_down:<?php print $menu_item['id'] ;  ?>">down</a></td>
        ...
        <? $unique_pic_id = 'pic_'.md5(rand()); ?>
        <form   action="<? print site_url('admin/menus/menus_save_menu_item') ?>"   method="post" id="menu_item_form_<?php print $unique_form_id ;  ?>" enctype="multipart/form-data">
          <table border="0" width="100%" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td><a href="#TB_inline?height=100&width=300&inlineId=<? print $unique_pic_id ; ?>&modal=false" class="thickbox">Add picture</a>
                <div id="<? print $unique_pic_id ; ?>" style="display:none">
                  <input name="picture_1" type="file"  />
                  <input type="button" name="Close" value="Close" onclick="javascript:tb_remove()"  />
                </div></td>
              ...
              <td><input name="item_parent" type="hidden" value="<?php print $item['id']; ?>">
                ...
                <a href="<? print site_url('admin/menus/menus_edit_small_menu_item') ?>/id:<?php print $unique_form_id ;  ?>/form:menu_item_form_<?php print $unique_form_id ;  ?>/?height=500&width=400&modal=false" class="thickbox" title="Change item">url</a>
