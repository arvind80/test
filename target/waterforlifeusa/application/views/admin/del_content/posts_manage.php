...
<table cellpadding="0" cellspacing="0" id="asdbtable">
  <tr>
    <td width="280px" style="padding-left: 74px"><div class="bar"> 
        ...
        <?
	 $link = site_url('admin/content/posts_manage/categories:'). '{id}';
	 ...
	 ?>
        <div class="ooyes_ul_tree_container" id="c31" style="float: left;clear: both" treeparams='<? print $tree_params_string;  ?>'></div>
        <? $tree_params_string = false;  ?>
        <!--[if IE 6]>
     <style>
     .treeparam_object{
        height: 1000px;
        ...
        <a href="#TB_inline?height=500&width=400&inlineId=create_new_content_choose_category_conpatiner&modal=false" class="bgb thickbox<?php if( $className == 'content' and $the_action == 'posts_add')  : ?> active<? endif; ?>"> Create new content </a> </div></td>
    <td><div id="side-content-wrapper">
        <div id="side-content">
          <div style="height: 15px;overflow: hidden;clear: both">&nbsp;</div>
          <form class="" id="searchengine" action="<? print site_url('admin/content/posts_manage_do_search_by_keyword/'); ?>" method="post">
            <label class="lbl search_label">Search<span></span></label>
            <input name="keyword" type="text" style="width:407px;" value="<? print $search_by_keyword ?>" />
            <div class="d"></div>
            <table cellpadding="2">
            ...
            <tr>
                <td><label class="ico"><img src="<?php print_the_static_files_url() ; ?>/icons/connect.png" alt=" " border="0">Type?</label>
                  <select name="content_subtype">
                    <option value=""  <? if($content_subtype == '') : ?> selected="selected" <? endif; ?> >Any</option>
                    <option value="trainings" <? if($content_subtype == 'trainings') : ?> selected="selected" <? endif; ?> >trainings</option>
                    <option value="products" <? if($content_subtype == 'products') : ?> selected="selected" <? endif; ?>>products</option>
                      <option value="services" <? if($content_subtype == 'services') : ?> selected="selected" <? endif; ?>>services</option>
                  </select></td>
                <td> </td>
              </tr>
              ...
              <tr>
                ...
                <option value="n" <? if($search_by_with_comments == 'n') : ?> selected="selected" <? endif; ?>>No</option>-->
                    ...
                    <option value=""  <? if($search_by_with_comments == false) : ?> selected="selected" <? endif; ?> >Any</option>
                    <?
$number = 365*2;
for ($x = 1; $x <= $number; $x++) : ?>
                    <option value="<? print $x ?> days"  <? if($search_by_with_comments ==  "$x days") : ?> selected="selected" <? endif; ?> ><? print $x ?> days</option>
                    <? endfor; ?>
                  </select></td>
              </tr>
              <tr>
...
$number = 365*2;
for ($x = 1; $x <= $number; $x++) : ?>
                    <option value="<? print $x ?> days"  <? if($selected_voted ==  "$x days") : ?> selected="selected" <? endif; ?> ><? print $x ?> days</option>
                    <?	endfor; ?>
                  </select></td>
...
for ($x = 1; $x <= $number; $x++) : ?>
            ...
            <!-- <div class="d"></div>
              ...
              <input name="tags" id="search_by_tags" type="text" style="width:80%;" value="<? print $content_selected_tags ?>" />-->
            <div class="d"></div>
            <div class="d"></div>
...
function batch_delete_all_items(){
 ...
 setInterval("opt_nav()", 100);
	...
	?>
              ...
              <label>
                <input name="content_parent" type="radio" value="<? print $section['id'] ?>"      />
                ...
                <span id="content_url_<? print $section['id'] ?>"><? print site_url($section['content_url']); ?></span> </label>
              <br />
              <? endforeach;  ?>
              <br />
              <h2>Categories</h2>
          ...
          <table class="tablesorter tables" id="sortedtable" cellspacing="0" cellpadding="0">
            ...
            <tbody id="the_content_items_container">
              ...
              <tr id="content_row_id_<? print $item['id']; ?>"> 
                <!-- ID -->
                <td style="display: none"><? print $item['id']; ?><a href="<? print $this->content_model->contentGetHrefForPostId($item['id']) ; ?>" target="_blank" ><img src="<?php print_the_static_files_url() ; ?>admin/icons/silk/page_go.png"  border="0" alt=" " /></a> <br />
                  <? print $item['id']; ?>
                  <input type="checkbox" value="<? print $item['id'] ?>" name="content_item_batch[]" /></td>
                <!-- Image -->
                <td style="display: none"><div class="cont_image" style="background-image: url('<? print   $thumb = $this->content_model->contentGetThumbnailForContentId($item['id'], 110); ?>')"></div></td>
                <td style="display: none"><?  //print $the_url.'/'.$item['content_url'] ?>
                  <? print $this->content_model->contentGetHrefForPostId($item['id']) ; ?></td>
                <td style="display: none"><? print ($item['content_title']) ?></td>
                <td style="display: none"><? print ($item['is_active']) ?></td>
                <td style="display: none"><? 	$temp= $this->content_model->commentsGetForContentId( $item['id']); print (count(  $temp )); ?></td>
                <td style="display: none"><? if($selected_voted == false) : ?>
                  <? $temp = 7 ?>
                  <? else: ?>
                  <? $temp = $selected_voted ?>
                  <? endif; ?>
                  <? print $this->content_model->contentGetVotesCountForContentId($item['id'], $temp); ?></td>
                <td style="display: none"><? print ($item['created_on']) ?></td>
                <td style="display: none"><? print ($item['updated_on']) ?></td>
                <td style="display: none"><a target="_blank" href="<?php print site_url('admin/content/posts_edit/id:'.$item['id'])  ?>">Edit post</a></td>
                <td style="display: none"></td>
                <td class="visibleCell"><div class="cont_image" style="background-image: url('<? print   $thumb = $this->content_model->contentGetThumbnailForContentId($item['id'], 150); ?>')"><a class="visit_post_link" href="<? print $this->content_model->contentGetHrefForPostId($item['id']) ; ?>" target="_blank" >Visit</a></div>
                  <div class="visibleCell_right"> <span class="visibleCell_right_comments">Comments
                    ...
                    <a class="deleteanchor"  href="javascript:deleteContentItem(<? print $item['id']; ?>, 'content_row_id_<? print $item['id']; ?>')">Delete</a> </div>
                  <div class="visibleCell_middle">
                    <h3><? print ($item['content_title']) ?></h3>
                    <span class="date">Created on:<? print ($item['created_on']) ?>; Updated on:<? print ($item['updated_on']) ?></span>
                    <div>
                      <? if($item['content_description'] != ''): ?>
                      <? print character_limiter($item['content_description'], 250, '...'); ?>
                      <? else: ?>
                      <? print character_limiter($item['content_body_nohtml'], 250, '...'); ?>
                      <? endif; ?>
                    </div>
                  </div></td>
              </tr>
