...
class taxonomy_model extends Model {
	...
	function taxonomySave($data, $preserve_cache = false) {
		...
		if (trim ( $data ['content_body'] ) != '') {
			$CI = get_instance();
			$data ['content_body'] = $CI->content_model->parseContentBodyItems ( $data ['content_body'], $data ['taxonomy_value'] );
	...
	function getToTableIds($root, $limit = false) {
		...
		$q = " SELECT id, parent_id,to_table_id from $table_taxonomy_items 
		where $root_q $visible_on_frontend_q and 
		taxonomy_type='category_item'  
