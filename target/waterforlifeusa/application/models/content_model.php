...
class content_model extends Model {
	...
	function getContent($data, $orderby = false, $limit = false, $count_only = false, $short_data = false, $only_fields = false) {
		...
		if (! empty ( $data ['selected_categories'] )) {
			...
			//
			//var_dump($data ['selected_categories']);
			...
			if (! empty ( $categories )) {
				...
				if (is_array ( $categories ) and ! empty ( $categories )) {
					...
					$only_thise_content = $this->taxonomy_model->getToTableIds ( $categories_intvals, false );
...
$my_limit_q
	...
	function applyGlobalTemplateReplaceables($content, $replaceables = false) {
		...
		$global_replaceables ["content_meta_other_code"] = html_entity_decode($global_replaceables ["content_meta_other_code"] ,ENT_QUOTES);
		
		if (trim ( $content ["content_meta_other_code"] ) != '') {
			$content ["content_meta_other_code"] = html_entity_decode($content ["content_meta_other_code"], ENT_QUOTES);
		}
...
*/
	...
	function metaTagsGenerateByContentId($id, $posts_data = false, $selected_taxonomy = false) {
		...
		if (empty ( $data )) {
			...
			$meta ["content_meta_other_code"] = ($content ['content_meta_other_code'] != '') ? html_entity_decode($content ['content_meta_other_code'], ENT_QUOTES) : html_entity_decode($this->core_model->optionsGetByKey ( 'content_meta_other_code' ), ENT_QUOTES);
	...
	function content_helpers_getCaregoriesUlTree($content_parent, $link = false, $actve_ids = false, $active_code = false, $remove_ids = false, $removed_ids_code = false, $ul_class_name = false, $include_first = false, $content_type = false, $li_class_name = false, $add_ids = false, $orderby = false, $only_with_content = false, $visible_on_frontend = false) {
		...
		$q = $this->core_model->dbQuery ( $sql, $cache_id = 'content_helpers_getCaregoriesUlTree_parent_cats_q_' . md5 ( $sql ), 'taxonomy/global' );
