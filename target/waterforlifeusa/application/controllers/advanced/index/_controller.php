...
if (defined ( 'INTERNAL_API_CALL' ) == true) {
	...
	//$this->output->set_output ( $layout );
		...
		//	p ( $page ,1 );
		//print '</pre>';
		if(!empty($post) and trim($post['content_meta_title']) != ''){
			$content ['content_meta_title'] = $post['content_meta_title'];
		} else {
			$content ['content_meta_title'] = $page['content_meta_title'];
		}
		
		if(!empty($post) and trim($post['content_meta_description']) != ''){
			$content ['content_meta_description'] = $post['content_meta_description'];
		} else {
			$content ['content_meta_description'] = $page['content_meta_description'];
		}
		
		if(!empty($post) and trim($post['content_meta_keywords']) != ''){
			$content ['content_meta_keywords'] = $post['content_meta_keywords'];
		} else {
			$content ['content_meta_keywords'] = $page['content_meta_keywords'];
		}
		
		
		if(!empty($post) and trim($post['content_meta_other_code']) != ''){
			$content ['content_meta_other_code'] = $post['content_meta_other_code'];
		} else {
			$content ['content_meta_other_code'] = $page['content_meta_other_code'];
		}
		...
		//p($global_template_replaceables);
