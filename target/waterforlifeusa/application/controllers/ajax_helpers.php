...
*/
	...
	function cart_orderPlace() {
		...
		if ($is_valid == true) {
			...
			$opt ['name'] = $_POST ['first_name'];
			
			$opt ['order_id'] = $_POST ['order_id'];
			
			//$this->cart_model->sendMailToBilling ( $opt, true );
			$site = site_url ();
		$object = 'Thank you for your order from WFL';
	 
		$message = "Hello, <b>{$opt['name']}</b>!

		 
		Thank you for your order from {$site} . 
		
		You have made payment for order id: <b>{$opt ['order_id']}</b>
...
$smo = array();
$smo['from_email'] = 'system@waterforlifeusa.com';
$smo['to_email'] = $opt ['email'];
$smo['subject'] = $object;
$smo['message'] = $message;
...
$this->core_model->sendMail2($smo);
	...
	function cart_check_cc() {
		...
		$to = 	$this->session->userdata ( 'shipping_user_email' );
		
		
		
		$am =  $this->cart_model->itemsGetTotal( $this->session->userdata ('cart_promo_code' ), $this->session->userdata ( 'shop_currency' ));
		$cur = $this->session->userdata ( 'shop_currency' );
		...
		$message = "
<div style=' width:300; height:100px; background:#0B669F;'><img src='https://waterforlifeusa.com/userfiles/templates/waterforlifeusa/img/logov2.png' /></div></br>Hello, 

		 
		Thank you for your order from {$site} . 
		
		You have made the payment for your order.</br>
		
		We will contact you shortly after we process your order</br>
		Below is the details of your order.</br>
		<p> Name :- {$this->session->userdata ( 'shipping_first_name' )} {$this->session->userdata ( 'shipping_last_name' )}</p>
		<p>Phone Number:-  {$this->session->userdata ( 'shipping_user_phone' )}</p>
		<p>Shipping Addesss:- {$this->session->userdata ( 'shipping_address' )}</p>
		<p>Shipping State:-  {$this->session->userdata ( 'shipping_state' )}</p>
		<p>Shipping city:- {$this->session->userdata ( 'shipping_city' )}</p>
		<p>Shipping Zip:-  {$this->session->userdata ( 'shipping_zip' )}</p>
		<p>Company Name:- {$this->session->userdata ( 'shipping_company_name' )}</p>
		<p>Shipping User Email:- {$this->session->userdata ( 'shipping_user_email' )}</p>
		<p>Shipping Service Type:- {$this->session->userdata ( 'shipping_service' )}</p>

		<p>Shipping Currency:- {$this->session->userdata ( 'shop_currency' )}</p>
		<p>Amount:-{$am}$</p>
		"; 
		
		
		
//send  the order receipt to customer		
$smo = array();
$smo['from_email'] = 'system@waterforlifeusa.com';
$smo['to_email'] = $to ;
$smo['subject'] = $object;
...
$headers = 'From: info@waterforlifeusa.com' . "\r\n" .
    'Reply-To: info@waterforlifeusa.com' . "\r\n" .
	'Content-type: text/html\r\n' . "\r\n" .
	 'Reply-To: info@waterforlifeusa.com' . "\r\n" .
	 'Cc: info@waterforlifeusa.com' . "\r\n".
