...
class FilterDef {
  ...
  function CreateJoinedComparison($Columns,$Values,$Comparison,$ImpliedAND,$ImpliedOR,$StartEnc,$EndEnc,$FieldType,$Wild,$DateSep,$dbtype) {
    if(strpos($ImpliedAND,"+") === false) {
      $ImpliedAND = str_replace("%20","",urldecode($ImpliedAND));
    }
...
function setQueryBuilderSource(&$RS_Source,$QB,$useOr)      {
  ...
  if ($QB->ObjectType == "WC") {
    ...
    if($theIndex[1]){
      $theClause = preg_replace("/^\s*WHERE\s{1,}/i", $theConnector." ", $theClause);
    }
    else{
      $theClause = "WHERE ".preg_replace("/^\s*(AND|OR)\s{1,}/i", " ", $theClause);
    }
  }
...
function findSourceInsertIndex($source)     {
  $returnIndex = 0;
  $indexOfLastFrom = (preg_match("/\s{1,}(FROM)\s{1,}/i",$source, $matches)>0) ? strpos($source,$matches[0])+1 : false;
