...
class PayPal
{
	...
	{	
		...
		if($this->Sandbox)
		{
			$file = $_SERVER['DOCUMENT_ROOT'].'/log/paypal/'.$timestamp.$filename.'.txt';	
		}
		else
		{
			$file = "/home/harvest/public_html/log/paypal/".$timestamp.$filename.".txt";
		}
		$fh = fopen($file, 'w');
...
class PayPal_Adaptive extends PayPal
{
	...
	{	
		...
		foreach($Receivers as $Receiver)
		{
			...
			$XMLRequest .= $Receiver['Primary'] != '' ? '<primary xmlns="">' . $Receiver['Primary'] . '</primary>' : '';
			$XMLRequest .= '</receiver>';
		}
