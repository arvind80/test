         ...
         if(is_array($valuesearch['SEARCHRESULTS'])){
               foreach($valuesearch['SEARCHRESULTS'] as $SearchResult){
                               $SenderTransactionID = isset($SearchResult['L_TRANSACTIONID']) ? $SearchResult['L_TRANSACTIONID'] : '';
                                ...
                                //code to convert the global time-stamp  to indian time-stamp.
                               $updatedTimestamp=$db->query_first("SELECT CONVERT_TZ('".$SearchResult['L_TIMESTAMP']."', '+00:00', '-5:00') as L_Time_");
                               $SearchResult['L_TIMESTAMP'] = $updatedTimestamp['L_Time_'];
                               $Transfer['L_TIMESTAMP'] = isset($SearchResult['L_TIMESTAMP']) ? $SearchResult['L_TIMESTAMP'] : '';

			       $SearchResult['L_TIMEZONE']='EST';
				...
				$Transaction['PAYERID'] = isset($PayPalResult['PAYERID']) ? $PayPalResult['PAYERID'] : '';
                              ...
                              // $Transaction['TIMESTAMP'] = isset($PayPalResult['TIMESTAMP']) ? $PayPalResult['TIMESTAMP'] : '';
                               ...
                               $Transaction['TAXAMT'] = isset($PayPalResult['TAXAMT']) && $PayPalResult['TAXAMT'] > 0 ? $PayPalResult['TAXAMT'] : 0;
                                ...
                                //code to convert the global standard time   to Eastern standard time.
                               unset($updatedTimestamp['L_Time_']);
                               $updatedTimestamp=$db->query_first("SELECT CONVERT_TZ('".$PayPalResult['TIMESTAMP']."', '+00:00', '-5:00') as L_Time_");
                               $PayPalResult['TIMESTAMP'] = $updatedTimestamp['L_Time_'];
                               $Transaction['TIMESTAMP'] = isset($PayPalResult['TIMESTAMP']) ? $PayPalResult['TIMESTAMP'] : '';
                               ...
                               //code to convert the global standard time   to Eastern standard time.
                               unset($updatedTimestamp['L_Time_']);
                               $updatedTimestamp=$db->query_first("SELECT CONVERT_TZ('".$PayPalResult['ORDERTIME']."', '+00:00', '-5:00') as L_Time_");
                               $PayPalResult['ORDERTIME'] = $updatedTimestamp['L_Time_'];
                               ...
                               //code to convert the global standard time   to Eastern standard time.
                               unset($updatedTimestamp['L_Time_']);
                               $updatedTimestamp=$db->query_first("SELECT CONVERT_TZ('".$PayPalResult['CLOSINGDATE']."', '+00:00', '-5:00') as L_Time_");
                               $PayPalResult['CLOSINGDATE'] = $updatedTimestamp['L_Time_'];
