...
if(!empty($_POST['dept']) && isset($_POST['dept'])){
	...
	if($_POST['dept']!='all'){
		...
		$StatusReport->getPdfData($_POST['dept'],$_POST['view_by_date']);
	}else{
	...
	//$dept=array('php','.net','iphone','seo','testing','BD','HR','design');
	...
	$pdf->SetHeaderData('logo.png', PDF_HEADER_LOGO_WIDTH, ucwords($departmentName).' Daily Report', 'http://www.kindlebit.com');
    ...
    //$displayImagePath="admin/".$image_path;
	$html='<div>
   ...
   //Total variable for particular user.
	...
	foreach($StatusReport->dailyReport as $val){
			...
			if(in_array($StatusReport->getDepartmentIdByUserId($val->user_id),$dept) && ($_POST['dept']=='all')){
				...
				$store.='<tr><td colspan="9" align="center"><h2><font color="green">'.ucwords($StatusReport->getDepartmentNameByUserId($val->user_id)).'</font></h2></td></tr>';
			}
			...
			if($previous_user_id==''){
				...
				$html.='</tr><tr><td colspan="9"></td></tr>';
			}elseif($previous_user_id!=$val->user_id){
				$previous_user_id=$val->user_id;
				$user_today_total=$user_free_hour+$user_working_hour+$user_hour_billed;
				$html.='<tr><td></td><td colspan="8" height="3px"><hr></td></tr>';
				$html.='<tr><td></td><td></td><td></td><td></td><td><strong>Todays Hours</strong></td><td></td><td></td><td></td><td><strong>'.$user_today_total.' hours</strong></td></tr>';
				$user_today_total=0;
				//check for new department.
				if($previousdepartment!=$StatusReport->getDepartmentIdByUserId($val->user_id)){
					$previousdepartment=$StatusReport->getDepartmentIdByUserId($val->user_id);
					
					$html.='<tr style="margin:10px;"><td colspan="9" align="center"><h2><font color="green">'.ucfirst($StatusReport->getDepartmentNameByUserId($val->user_id)).'</font></h2></td></tr>';
				}
			...
			$html.='<tr><td></td>';
			$html.='<td align="center"><a href="http://target.kindlebit.com/viewdescription.php?id='.$val->id.'">'._($val->project_name).'</a></td>';
			$html.='<td align="center">'.$val->project_type.'</td>';
			$html.='<td align="center">'.$val->odesk_id.'</td>';
			$html.='<td align="center">'.$val->client_name.'</td>';
			$html.='<td align="center">'.$val->working_hour.'</td>';
			$html.='<td align="center">'.$val->hour_billed.'</td>';
			$html.='<td align="center">'.$val->estimated_hour.'</td>';
	...
	$html.='<tr><td colspan="5" style="text-align:center;"><strong>Total Hours</strong></td><td><strong>'.$working_hour.'hours</strong></td><td><strong>'.$hour_billed.'hours</strong></td><td><strong>'.$estimated_hour.'hours</strong></td><td><strong>'.$free_hour.' hours</strong></td></tr>';
	$html.='</table></div>';
	...
	$pdf->Output($_POST['dept'].' daily_report_'.date('Y-m-d').'.pdf', 'I');
}
?>
