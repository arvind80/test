	...
	function onAcyDisplayActions(&$type){
	 ...
	 function onAcyDisplayFilters(&$type){
		...
		foreach($lists as $oneList){
	 	...
	 	$filter = '<div id="filter__num__list">'.$status->display("filter[__num__][list][status]",1,false).' '.JHTML::_('select.genericlist',   $listsdrop, "filter[__num__][list][selectedlist]", 'class="inputbox" size="1" onchange="countresults(__num__)"', 'value', 'text');
...
<?php
	...
	function _listcount(&$email,&$user,&$parameter){
		...
		if(empty($listid)){
			...
			$db->setQuery('SELECT COUNT(subid) FROM #__acymailing_listsub WHERE listid = '.$listid.' AND status = 1');
		}
	...
	function replaceSubscriptionTag(&$allresults,$i,&$user,&$email){
		...
		if($allresults[1][$i] == 'confirm'){ //confirm your subscription link
			...
			$myLink = acymailing_frontendLink('index.php?option=com_acymailing&ctrl=user&task=confirm&subid='.$user->subid.'&key='.$user->key.$item,(bool) $this->params->get('confirmtemplate',false));
			...
			if(!empty($itemId)) $item = '&Itemid='.$itemId;
			$myLink = acymailing_frontendLink('index.php?option=com_acymailing&ctrl=user&task=modify&subid='.$user->subid.'&key='.$user->key.$item,(bool) $this->params->get('modifytemplate',false));
			if(empty($allresults[2][$i])) return $myLink;
		...
		if(!empty($itemId)) $item = '&Itemid='.$itemId;
		$myLink = acymailing_frontendLink('index.php?option=com_acymailing&ctrl=user&task=optout&mailid='.$email->mailid.'&subid='.$user->subid.'&key='.$user->key.$item,(bool) $this->params->get('unsubscribetemplate',false));
		if(empty($allresults[2][$i])) return $myLink;
		return '<a style="text-decoration:none;" target="_blank" href="'.$myLink.'"><span class="acymailing_unsub">'.$allresults[2][$i].'</span></a>';
	}
}//endclass